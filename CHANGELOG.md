...
### Fixed
- Fixed authentication with Helicone integration for OpenRouter API
- Updated settings to properly handle API keys for both OpenRouter and Helicone
- Ensured proper header formatting for Helicone-Auth in API requests

## [0.4.1] - 2025-05-01

### Fixed
- Modified `extract_xml_tag` function in OpenRouterService to preserve the original case of extracted content by removing the `.lower()` call
